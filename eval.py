...
def run_test():
    ...
    ds = create_yolox_dataset(image_dir=data_root, anno_path=annFile, is_training=False,
                              batch_size=args.per_batch_size, device_num=args.group_size, rank=args.rank, exp_name=exp_name)
    data_size = ds.get_dataset_size()

    # 设置device
    context.set_context(mode=context.GRAPH_MODE, device_target=args.device_target, save_graphs=False)
    # 定义网络
    network = YOLOX(config, backbone=args.backbone)
    ...
    if args.singel_eval:
        ...
        get_result(network, ds, data_size, config, logger)
    else:
...
def get_result(network, ds, data_size, config, logger):
