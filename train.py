...
from yolox.models.yolox_dataset import create_yolox_dataset
from yolox.models.losses import YOLOLossCell
from yolox.models.yolox import YOLOX
from yolox.exp.yolo_base import YOLOXCB
from yolox.config import get_config
...
def get_parser():
    ...
    parser.add_argument('--name', type=str, default="yolox-s", help='model name, yolox-s, yolox-m, yolox-l, yolox-x')
...
def run_train():
    ...
    ds = create_yolox_dataset(image_dir=data_root, anno_path=annFile, batch_size=args.per_batch_size,
                              device_num=args.group_size, rank=args.rank, data_aug=args.data_aug, exp_name=exp_name)
    steps_per_epoch = ds.get_dataset_size()
    # 定义网络
    base_network = YOLOX(config, backbone=args.backbone)
    ...
    model.train(args.max_epoch, ds, callbacks=cb, dataset_sink_mode=False, sink_size=-1)
